-- One CSV row per athlete (or line) in an event; result_id is shared by many rows (e.g. same heat).
CREATE TABLE IF NOT EXISTS athlete_events (
    athlete_event_id BIGSERIAL PRIMARY KEY,
...
CREATE INDEX IF NOT EXISTS idx_athlete_events_result_id ON athlete_events (result_id);
